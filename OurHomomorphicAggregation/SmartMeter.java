...
public class SmartMeter {
	...
	public SmartMeter(ECC_Parameters ps) throws IOException {
		super();
	...
	public RepMessage genRepMessage(int reportingDataType) throws IOException {

		BigInteger mi, tem, c = BigInteger.ONE;
		for (int i = 0; i < reportingDataType; i++) {
			mi = BigInteger.valueOf(Utils.randomInt());
			tem = gg[i].modPow(mi, nsquare);
			c = (c.multiply(tem)).mod(nsquare);
			tem = gg[i + reportingDataType].modPow(mi.multiply(mi), nsquare);
			c = (c.multiply(tem)).mod(nsquare);
		}
