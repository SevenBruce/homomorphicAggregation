...
public class Utils {
	...
	public static Element hash2Element(String input,Pairing pairing) throws IOException {
		Element rid = pairing.getG1().newElementFromHash(input.getBytes(),0,input.length());
		return rid;
	}
...
//	}
	 ...
	 * @return long
	 */
	public static BigInteger randomLengthBig(int length) {
		Random rnd = new Random();
		long seed = System.nanoTime();
		rnd.setSeed(seed);
		BigInteger ranBig = new BigInteger(length, rnd);
		return ranBig;
	}
